...
3. For every manual step in each scenario's `procedure`, write ONE independent
   script under `.testkit/work/<feature>/scripts/` (use
   `.testkit/scripts/powershell/_template.ps1`) that does exactly that one thing
   and mimics the user's manual action. The work dir is gitignored; scripts are
   regenerated from `test-memory.md` on every Discover run and must never be
   committed.
...
- `.testkit/work/<feature>/scripts/*.ps1` — one small script per manual step.
  **Gitignored, never committed.** Regenerate by re-running `/testkit.discover`.
- `<spec-dir>/tools-report.md` from `.testkit/templates/tools-report.md`: the
  reachability table, plus per scenario id the script paths (relative to repo
  root) and run order.
